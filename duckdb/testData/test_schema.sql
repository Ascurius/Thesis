...
CREATE TABLE mi_cohort AS (
       SELECT DISTINCT patient_id FROM diagnoses WHERE icd9 LIKE '410%'  AND year = 2006 AND (site=4 OR site=7));



 -- data used in test to simulate patient registry
DROP TABLE IF EXISTS cdiff_cohort_diagnoses;
CREATE TABLE cdiff_cohort_diagnoses AS SELECT * FROM diagnoses WHERE year = 2006 AND (site=4 OR site=7) AND patient_id IN (SELECT * FROM cdiff_cohort); 
DROP TABLE IF EXISTS mi_cohort_diagnoses;
CREATE TABLE mi_cohort_diagnoses AS SELECT * FROM diagnoses  WHERE year = 2006 AND (site=4 OR site=7) AND patient_id IN (SELECT * FROM mi_cohort); 
...
CREATE TABLE mi_cohort_medications AS SELECT * FROM medications  WHERE year = 2006 AND (site=4 OR site=7) AND patient_id IN (SELECT * FROM mi_cohort); 
